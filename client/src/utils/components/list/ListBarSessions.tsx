...
function SessionStatusIcon ({ status, data, sessionId, errorSession, defaultImage }: SessionStatusIconProps) {
  ...
  return (
    <div id={sessionId}
      ...
      <span className={`text-${data.color} small`}>{data.text}</span>
      {popover}
    </div>);
}
...
function ListBarSession(
    ...
    client.getCommits(id, notebook.annotations.branch).then(
      (commitsFetched: any) => {
        if (commitsFetched.data?.length > 0) {
          const sessionCommit =
            commitsFetched.data.filter((commit: any) => commit.id === notebook.annotations["commit-sha"]);
  ...
  return (
    <div className="container-sessions" data-cy="container-session">
      ...
      <div className="session-resources"><ResourceList resources={resources} /></div>
