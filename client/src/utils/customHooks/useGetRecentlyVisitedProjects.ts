...
function useGetRecentlyVisitedProjects(projectsCount: number, currentSessions: Session[]) {
  ...
  if (!isFetching && data.length > 0 && currentSessions.length > 0) {
    const sessionProjectIds = currentSessions.map( (session: any) => session.annotations["gitlabProjectId"]);
    projectsToShow = data
      .filter((project: any) => !sessionProjectIds?.includes(`${project.id}`)).splice(0, totalProjectsToReturn);
  }
  return {
