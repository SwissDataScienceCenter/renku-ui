...
} from "../../features/session/sidecarApi";
...
function SaveSession(props: SaveSessionProps) {
  ...
  return (
    <Modal className="modal-session" isOpen={isOpen} toggle={closeModal}>
      <ModalHeader toggle={closeModal}>Save Session</ModalHeader>
...
function RunningSaveSessionBody(props: SaveSessionProps) {
  ...
  else if (!props.hasSaveAccess)
    body = <NoSaveAccessBody closeModal={closeModal} />;
  else if (isLoading) body = <CenteredLoader />;
  else if (
    error != null ||
    data == null ||
    data.status !== "running" ||
...
function SaveSessionStatusBody({
  closeModal,
  isOpen,
  sessionName,
}: SaveSessionStatusBodyProps) {
  ...
  if (succeeded === false)
    return <SaveSessionFailedBody closeModal={closeModal} />;
  if (succeeded === true)
      ...
      <FormGroup>
        ...
        <Input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
            props.setCommitMessage(e.target.value);
          }}
        ...
        <Fragment>
          {saving ? (
            <FormText color="primary">
              <Loader inline size={16} /> Saving Session
              <br />
            </FormText>
          ) : null}
