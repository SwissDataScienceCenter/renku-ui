...
describe("test version status", () => {
  it("handles up-to-date", () => {
    const result = migrationCheckToRenkuVersionStatus(
      exampleMigrationChecks.current
    );
    expect(result.renkuVersionStatus).toEqual(
      RENKU_VERSION_SCENARIOS.RENKU_UP_TO_DATE
    );
  });
  it("handles not supported", () => {
    const result = migrationCheckToRenkuVersionStatus(
      exampleMigrationChecks.notSupported
    );
    expect(result.renkuVersionStatus).toEqual(
  ...
  it("handles new version not required", () => {
    const result = migrationCheckToRenkuVersionStatus(
      exampleMigrationChecks.notRequired
    );
    expect(result.renkuVersionStatus).toEqual(
  ...
  it("handles new version not required, auto-update possible", () => {
    const result = migrationCheckToRenkuVersionStatus(
      exampleMigrationChecks.requiredAuto
    );
    expect(result.renkuVersionStatus).toEqual(
  ...
  it("handles new version not required, auto-update not possible", () => {
    const result = migrationCheckToRenkuVersionStatus(
      exampleMigrationChecks.requiredManual
    );
    expect(result.renkuVersionStatus).toEqual(
      RENKU_VERSION_SCENARIOS.NEW_VERSION_REQUIRED
    );
...
describe("test display warning", () => {
  it("handles up-to-date", () => {
    const result = shouldDisplayVersionWarning({
      check: exampleMigrationChecks.current,
    });
    expect(result).toEqual(false);
  });
  it("handles not supported", () => {
    const result = shouldDisplayVersionWarning({
      check: exampleMigrationChecks.notSupported,
    });
    expect(result).toEqual(true);
  });
  it("handles new version not required", () => {
    const result = shouldDisplayVersionWarning({
      check: exampleMigrationChecks.notRequired,
    });
    expect(result).toEqual(false);
  });
  it("handles new version not required, auto-update possible", () => {
    const result = shouldDisplayVersionWarning({
      check: exampleMigrationChecks.requiredAuto,
    });
    expect(result).toEqual(true);
  });
  it("handles new version not required, auto-update not possible", () => {
    const result = shouldDisplayVersionWarning({
      check: exampleMigrationChecks.requiredManual,
    });
    expect(result).toEqual(true);
  });
});
