...
function getTemplateVersionStatus({
  ...
  if (!project_supported)
    return TEMPLATE_VERSION_SCENARIOS.PROJECT_NOT_SUPPORTED;
  if (template_id === null)
    return TEMPLATE_VERSION_SCENARIOS.TEMPLATE_NOT_VERSIONED;
  if (!newer_template_available)
    return TEMPLATE_VERSION_SCENARIOS.TEMPLATE_UP_TO_DATE;
...
function TemplateUpdateSection({
  ...
  const docUrl = Docs.rtdPythonReferencePage(
    "commands.html#module-renku.ui.cli.migrate"
  );
  ...
  const automaticUpdateAction = maintainer ? (
    ...
    <p>
      <strong>
        You do not have the required permissions to update this projects
        template.
      </strong>
      &nbsp;You can{" "}
      <ExternalLink
  ...
  const UpdateActions = (
    <Fragment>
      <p>
        If you wish to take advantage of new features, you can update to the
        latest version of the <strong>template</strong>.
        {/**
      ...
      <Button
        color="link"
        className="ps-0 mb-2 link-alert-warning text-start"
        onClick={toggleOpen}
      >
        {projectTemplateStatus ===
        TEMPLATE_VERSION_SCENARIOS.NEW_TEMPLATE_AUTO ? (
          <i>Do you prefer manual instructions?</i>
        ) : (
          <i>
            Automated update is not possible, but you can follow these
            instructions to update manually.
          </i>
        )}
      </Button>
...
function TemplateSource({ template_source }) {
  ...
  if (components.length < 2) return template_source;
...
function TemplateVersionInfo({ check }) {
  ...
  const {
    ...
    template_ref,
    template_source,
  } = check.template_status;

  if (template_id == null) return null;
  return newer_template_available ? (
    <p>
      <strong>Template</strong>{" "}
      <TemplateSource template_source={template_source} /> &gt; {template_id}{" "}
      <br />
...
function TemplateVersionBody({
  ...
  switch (projectTemplateStatus) {
    case TEMPLATE_VERSION_SCENARIOS.PROJECT_NOT_SUPPORTED:
      return (
        <MigrationWarnAlert>
          Automatic upgrading of the <strong>template</strong> version is not
...
function GuardedTemplateVersionBody({
  externalUrl,
  launchNotebookUrl,
  lockStatus,
  logged,
