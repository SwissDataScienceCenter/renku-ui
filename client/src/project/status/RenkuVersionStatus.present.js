...
function updateNotRequired(renkuVersionStatus) {
  return (
    renkuVersionStatus === RENKU_VERSION_SCENARIOS.NEW_VERSION_NOT_REQUIRED
  );
}
...
function MigrationTimeEstimate({
  ...
  const migrationDurS = commitCountToMigrationTimeSecs(
    statistics.data.commit_count
  );
  // remove 'ago' from the duration
  const humanMigrationDur = human(migrationDurS).slice(0, -4);
  return <span>The update should take about {humanMigrationDur}.</span>;
}
  ...
  if (updateNotRequired(renkuVersionStatus)) {
    return (
      <div>
        ...
        If you wish to take advantage new features, you can update to the latest
        version.
        <MigrationTimeEstimate
          current_metadata_version={current_metadata_version}
          project_metadata_version={project_metadata_version}
          statistics={statistics}
        />
      </div>
    );
  }
  const updateMessage = backendAvailable ? null : (
    <p className="lh-sm">
      You can launch and work in interactive sessions, but to make changes to
      the project from the UI you will need to{" "}
      <strong>update the renku version</strong>.
    </p>
  );
  return (
    <div>
...
function AutoUpdateButton({
  ...
  return (
    <p className="lh-sm">
      You do not have the required permissions to update this project, but you
      can <AskMaintainer externalUrl={externalUrl} /> to do that for you.
    </p>
  );
}
  ...
  const updateDetails = maintainer ? (
    <Fragment>
      ...
      <UpdateSection>
        <Button
          color="link"
          className={`ps-0 mb-2 ${linkClassName} text-start`}
          onClick={toggleOpen}
        >
  ...
  return (
    <Fragment>
      ...
      {maintainer ? (
        ...
        <p className="lh-sm">
          You do not have the required permissions to update this project, but
          you can <AskMaintainer externalUrl={externalUrl} /> to do that for
          you.
        </p>
      )}
    </Fragment>
...
function RenkuVersionStatusBody({
  ...
  const current_metadata_version =
    migration?.check?.core_compatibility_status?.current_metadata_version;
  const project_metadata_version =
    migration?.check?.core_compatibility_status?.project_metadata_version;
  const updateProps = {
    backendAvailable,
    current_metadata_version,
    externalUrl,
  ...
  switch (renkuVersionStatus) {
    ...
    case RENKU_VERSION_SCENARIOS.NEW_VERSION_NOT_REQUIRED:
      body = <MigrationInfoAlert>{updateSection}</MigrationInfoAlert>;
      break;
    case RENKU_VERSION_SCENARIOS.NEW_VERSION_REQUIRED:
      message = backendAvailable ? (
        <div>
          ...
          .
          <p className="lh-sm">
            Although this project is compatible with the RenkuLab UI, it is
            using an older version of renku.
          </p>
        </div>
      ) : (
    ...
    case RENKU_VERSION_SCENARIOS.PROJECT_NOT_SUPPORTED:
      body = (
        <MigrationWarnAlert>
          This project appears to be using an experimental version of
          Renku.&nbsp;
          <br />
          <br />
          You can launch and work in interactive sessions, but creating or
          modifying datasets&nbsp; are not supported from the UI.
        </MigrationWarnAlert>
      );
      break;
    default:
...
function RenkuVersionInfo({ migration }) {
  ...
  const { project_metadata_version, current_metadata_version } =
    migration.check.core_compatibility_status;

  return newer_renku_available === true || newer_renku_available == null ? (
    <table style={{ borderSpacing: "10px 5px" }}>
      <tbody>
        <tr>
          <th className="mr-2" scope="row">
            Project Renku Version
          </th>
          <td className="mr-2">
            <span id="project_version">{shownRenkuVersion}</span>
          </td>
          <td style={{ fontSize: "smaller" }}>(v{project_metadata_version})</td>
        </tr>
        <tr>
          <th scope="row">Latest Renku Version</th>
          <td>{core_renku_version}</td>
          <td style={{ fontSize: "smaller" }}>(v{current_metadata_version})</td>
        </tr>
      </tbody>
    </table>
  ) : (
...
function RenkuVersionStatus(props) {
  ...
  if (isMigrationFailure({ check_error, migration_error, migration_status })) {
    let error;
    if (CoreError.isValid(migration_error)) error = migration_error;
    else if (CoreError.isValid(check_error)) error = check_error;
    else return null;
