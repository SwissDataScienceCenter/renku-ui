...
function DatasetChange(props) {
    ...
    return (
      <Col sm={12} md={10} lg={8}>
        <Alert timeout={0} color="primary">
          Acces Denied. You don&apos;t have rights to create datasets for this
          project.
          <br />
          <br />
  ...
  const formatServerErrorsAndWarnings = (errorOrWarning, isError) => {
    ...
    if (
      !isError &&
      (jobsStats.failed || jobsStats.inProgress || jobsStats.tooLong)
    ) {
      const failed = jobsStats.failed.map((job) => (
        <div key={"warn-" + job.file_url} className="pl-2">
          - {job.file_url}
          <br />
        </div>
      ));
      const progress = jobsStats.inProgress.map((job) => (
        <div key={"warn-" + job.file_url} className="pl-2">
          - {job.file_url}
          <br />
        </div>
      ...
      return (
        <div>
          {jobsStats.tooLong ? (
            <div>
              This operation is taking too long and it will continue being
              processed in the background.
              <br />
              Please check the datasets list later to make sure that the changes
              are visible in the project. <br />
              You can also check the{" "}
              <Link to={props.overviewCommitsUrl}>commits list</Link> in the
              project to see if commits for the new dataset appear there.
              <br />
              <br />
            </div>
          ) : null}
