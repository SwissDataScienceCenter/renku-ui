...
function ChangeDataset(props) {
  if (dsFormSchema == null) dsFormSchema = _.cloneDeep(datasetFormSchema);
  ...
  const initializeFunction = (formSchema) => {
    ...
    if (props.edit === false) {
      ...
      image.value = {
        options: [
          { [Prop.URL]: props.dataset ? props.dataset.mediaContent : null },
        ],
        selected: 0,
      };
    }
    ...
    fileField.uploadThresholdSoft = props.params.UPLOAD_THRESHOLD
      ? props.params.UPLOAD_THRESHOLD.soft
      : 104857600;
    ...
    if (!fileField.notifyFunction) {
      fileField.notifyFunction = (success = true, error) => {
        ...
        const redirectUrl = props.edit
          ? `/projects/${props.projectPathWithNamespace}/datasets/${dataset.name}/modify`
  ...
  function setNewJobStatus(localJob, remoteJobsList) {
    let remoteJob = remoteJobsList.find(
      (newJob) => newJob.job_id === localJob.job_id
    );
  ...
  function checkJobsAndSetWarnings(jobsList, tooLong = false, handlers) {
    let failed = jobsList.filter(
      (job) => job.job_status === JobStatusMap.FAILED
    );
    let inProgress = jobsList.filter(
      (job) =>
        job.job_status === JobStatusMap.IN_PROGRESS ||
        job.job_status === JobStatusMap.ENQUEUED
    );

    if (failed.length !== 0 || inProgress.length !== 0 || tooLong === true) {
      const jobStats = { failed, inProgress, tooLong };
      handlers.setDisableAll(true);
      handlers.setSecondaryButtonText(
        props.edit ? "Go to dataset" : "Go to list"
      );
  ...
  function getJobsStats(jobsList) {
    const failed = jobsList.filter(
      (job) => job.job_status === JobStatusMap.FAILED
    );
    const completed = jobsList.filter(
      (job) => job.job_status === JobStatusMap.COMPLETED
    );
  ...
  const monitorURLJobsStatuses = (datasetsJobsArray) => {
    return props.client.getAllJobStatus(versionUrl).then((response) => {
      //we set the new status and then we check if they are all finished (completed or failed)
      datasetsJobsArray.map((localJob) =>
        setNewJobStatus(localJob, response.jobs)
      );
  ...
  const submitCallback = async (e, mappedInputs, handlers) => {
    ...
    const submitLoaderText = props.edit
      ? "Modifying dataset, please wait..."
    ...
    dataset.creators = mappedInputs.creators.map((creator) =>
      getCreator(creator)
    );

    if (mappedInputs.image.options.length) {
      const imageSelected =
        mappedInputs.image.options[mappedInputs.image.selected];
      if (imageSelected.STOCK === false) {
        const images = await uploadDatasetImages(mappedInputs.image, handlers);
        if (images) dataset.images = images;
      } else {
        if (
          mappedInputs.image.options[mappedInputs.image.selected]?.URL != null
        ) {
          dataset.images = [
            {
              content_url:
                mappedInputs.image.options[mappedInputs.image.selected]?.URL,
              position: 0,
              mirror_locally: true,
            },
          ];
        ...
        if (response.data.error !== undefined) {
          ...
          if (error.errorOnFileAdd) {
            props.edit
              ? handlers.setServerErrors(
                  <div>
                    <p>
                      Metadata changes were successfully applied, but some files
          ...
          handlers.setServerWarnings(
            <div>
              ...
              <strong>{response.data.result.remote_branch}</strong> into{" "}
  ...
  useEffect(() => {
    ...
    if (props.edit) {
            ...
            if (!unmounted && datasetFiles === undefined) {
              if (response.data.result) {
                ...
                dsFormSchema.creators.value = dataset.published.creator.map(
                  (creator) =>
                    creator.email === props.user.data.email
                      ? {
                          name: creator.name,
                          email: creator.email,
                ...
                if (
                  dsFormSchema.creators.value.find(
                ...
                dsFormSchema.files.value = response.data.result.files.map(
                  (file) => ({
                    name: file.name,
      ...
      setInitialized(true);
      dsFormSchema.name.value =
        dsFormSchema.name.value === undefined
          ? dsFormSchema.name.initial
          : dsFormSchema.name.value;
