...
function getNodeLabel(node, NODE_COUNT, lineagesUrl) {
  if (node.type === "ProcessRun") {
    ...
    const smallerArray =
      stringArray.length > 4 // remove when too many arguments
        ? stringArray
            .slice(0, 1)
            .concat(MISSING_PIECES)
            .concat(stringArray.slice(stringArray.length - 3))
        : stringArray;
    const shortenArray = smallerArray.map((e) =>
      cropLabelStart(LABEL_LIMIT, e.trim())
    ); // crop long labels
    const label = shortenArray.join(BREAKING_LINE); // break line at each arg

    return (
      '<text><tspan xml:space="preserve" dy="1em" x="1">' +
      label +
      "</tspan></text>"
    );
  }
  ...
  if (node.type === "Directory" || node.type === "File") {
    ...
    return (
      '<text><tspan xml:space="preserve" dy="1em" x="1" data-href=' +
      ref +
      ">" +
...
function nodeToClass(node, currentNodeId, label) {
  ...
  if (nodeId === currentNodeId) nodeClasses.push("central");
  else nodeClasses.push("normal");
  ...
  if (node.type === "Directory" || node.type === "File") {
    if (
      node.location.includes(".") &&
      FORMATS[node.location.split(".").pop().toLowerCase()]
    )
      nodeClasses.push("code");
    else nodeClasses.push("data");
  } else {
...
class FileLineageGraph extends Component {
  ...
  componentDidMount() {
    if (this.subGraph()._nodeCount > 1) this.renderD3();
  }
  ...
  hasLink(nodeId, currentNodeId) {
    return this.props.graph.nodes.filter(function (node) {
      return (
        node.id === nodeId &&
  ...
  renderD3() {
    ...
    d3.selectAll("g.node")
      .filter((d) => {
        return this.hasLink(d, this.props.currentNode.id);
      })
      .select("tspan")
    ...
    // Center the graph
    const bBox = document
      .getElementsByClassName("graphContainer")[0]
      .lastChild.getBBox();
    ...
    return this.subGraph()._nodeCount > 1 ? (
      <div className="graphContainer" ref={(r) => (this._vizRoot = r)}>
        <div className="float-right zoomButtons">
          <button className="btn btn-light btn-group-left" id="zoom_in">
            +
          </button>
          <button className="btn btn-light btn-group-right" id="zoom_out">
            -
          </button>
        </div>
        <svg>
  ...
  render() {
    ...
    return (
      <Card>
        ...
        <CardBody className="scroll-x">{graphComponent}</CardBody>
      </Card>
    );
  }
}
