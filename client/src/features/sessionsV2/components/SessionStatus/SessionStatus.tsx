...
export function SessionStatusV2Label({ session }: ActiveSessionV2Props) {
  ...
  const badge =
    state === "running" && defaultImage ? (
      <SessionBadge className="border border-warning bg-rk-warning-100">
        <ExclamationCircleFill className="text-warning" size={16} />
        <span className="rk-bg-warning">Running Session</span>
      </SessionBadge>
    ) : state === "running" ? (
      <SessionBadge className="border border-success bg-rk-success-100">
        <CheckCircleFill className="text-success" size={16} />
        <span className="text-rk-text-success">Running Session</span>
      </SessionBadge>
    ) : state === "starting" ? (
      <SessionBadge className="border border-warning bg-rk-warning-100">
      ...
      <SessionBadge className={"border border-rk-gray-600 bg-rk-gray-200"}>
        <PauseCircleFill className="text-rk-gray-600" size={16} />
        <span className="text-dark ml-2">Paused Session</span>
      </SessionBadge>
    ) : state === "failed" ? (
      <SessionBadge className={"border border-danger bg-rk-danger-100"}>
