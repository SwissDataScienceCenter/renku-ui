...
export default function useSessionLauncherState({
  ...
  const sessionImage = dataSessionImage ?? { accessible: true };
  ...
  useEffect(() => {
    ...
    if (!isExternalImageEnvironment || sessionImage.accessible) {
      dispatch(startSessionOptionsV2Slice.actions.setImageReady(true));
    }
  }, [dispatch, isExternalImageEnvironment, sessionImage.accessible]);
