...
import { MigrationStatus, MigrationStatusParams, MigrationStatusResponse } from "./Project";


export const projectVersionApi = createApi({
  reducerPath: "projectVersion",
  ...
  endpoints: (builder) => ({
    getMigrationStatus: builder.query<MigrationStatus, MigrationStatusParams>({
      ...
      transformResponse: (response: MigrationStatusResponse) => {
        ...
        else if (response.result?.core_compatibility_status?.type === "error") {
          transformedResponse.errorProject = true;
          transformedResponse.details = response.result;
          transformedResponse.error = response.result?.core_compatibility_status;
        }
...
export const { useGetMigrationStatusQuery } = projectVersionApi;
