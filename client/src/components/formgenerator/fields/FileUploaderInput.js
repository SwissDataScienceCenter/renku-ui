...
function FileUploaderInput({
  ...
  const [errorOnDrop, setErrorOnDrop] = useState(
    internalValues ? internalValues.errorOnDrop : ""
  );
  const [initialized, setInitialized] = useState(
    internalValues ? internalValues.initialized : false
  );
  const [initialFilesTree, setInitialFilesTree] = useState(
    internalValues ? internalValues.initialFilesTree : undefined
  );
  const [partialFilesPath, setPartialFilesPath] = useState(
    internalValues ? internalValues.partialFilesPath : ""
  );
  const [urlInputValue, setUrlInputValue] = useState(
    internalValues ? internalValues.urlInputValue : ""
  );
  ...
  const onUrlInputEnter = (e) => {
    if (!disabled && (e.key === "Enter" || e.target.id === "addFileButton")) {
      e.preventDefault();
      if (!urlInputValue) setErrorUrlInput(INPUT_URL_ERROR.INVALID);
  ...
  const updateAndSetDisplayFilesAfterChanges = (file) => {
    const displayFiles = getCurrentFiles().map((dFile) =>
      dFile.file_name === file.file_name ? file : dFile
    );
  ...
  const setInputsInForm = () => {
    ...
    const artificialEvent = {
      target: {
        name: name,
        value: internalValues.displayFiles.filter((f) =>
          [FILE_STATUS.UPLOADED, FILE_STATUS.PENDING].includes(f.file_status)
        ),
  ...
  const getCurrentFiles = () => {
    const dFiles = handlers.getFormDraftInternalValuesProperty(
      "files",
      "displayFiles"
    );
  ...
  const setDisplayFilesRx = (newDisplayFiles) => {
    return handlers.setFormDraftInternalValuesProperty(
      "files",
      "displayFiles",
      newDisplayFiles
    );
  ...
  const notifyUpload = (file, extras) => {
    if (file.file_status === FILE_STATUS.UPLOADED) notifyFunction(true);
  ...
  const deleteFile = (file_name) => {
    if (!disabled) {
      ...
      if (currentFile) {
        setDisplayFilesRx(
          getCurrentFiles().filter((file) => file.file_name !== file_name)
        );
        setInputsInForm();
      }
    }
  ...
  const cancelUpload = (uploadId) => {
    ...
    if (fileToDelete.length) dropzone.removeFile(fileToDelete[0]);
  };
  ...
  const getFileStatusComp = (file) => {
    switch (file.file_status) {
      ...
      case FILE_STATUS.UPLOADED:
        return (
          <span>
            <FontAwesomeIcon color="var(--bs-success)" icon={faCheck} /> Ready
            to add
          </span>
        );
            ...
            {file.file_size >= uploadThresholdSoft ? (
              <small>
                <span className="text-muted">
                  {" "}
                  No need to wait. We will{" "}
                  <Link to="/notifications">notify you</Link> when the upload is
                  finished.
                </span>
              </small>
            ) : null}
  ...
  const getActions = (file, index) => {
    if (
      ...
      const retryButton =
        file.file_status === FILE_STATUS.FAILED ? (
          <div id={"retry-" + index}>
            <FontAwesomeIcon
              style={{ cursor: "pointer" }}
              icon={faSyncAlt}
      ...
      return (
        <span
          ...
          style={{ whiteSpace: "nowrap", cursor: "pointer" }}
          onClick={() => cancelUpload(file.upload_id)}
        >
          Cancel
        </span>
      );
      ...
      <tbody className={disabled ? "disabled-input" : ""}>
        {currentFiles.map((file, index) => (
          ...
          >
              ...
              {file.file_alias ? (
                <small>
                  ...
                  <span className="text-danger">
                    *The name of this file contains disallowed characters; it
                    has been renamed to <i> {file.file_alias}</i>
                  </span>
                </small>
              ) : null}
            ...
            <td className="text-center">{getActions(file, index)}</td>
          </tr>
        ))}
      </tbody>
    </Table>
  ...
  const getAlert = (alert) => {
    if (
      ...
      getCurrentFiles().filter(
        (f) => f.file_uncompress === FILE_COMPRESSED.WAITING
      ).length
    )
