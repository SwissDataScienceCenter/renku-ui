...
describe("Set up project components", () => {
  ...
  it("maintain session launcher env variables", () => {
    cy.intercept("/api/data/sessions*", {
      body: [],
    }).as("getSessionsV2");
    fixtures
    ...
    cy.getDataCy("edit-env-variables-input").clear().type("TEST=1");
    cy.getDataCy("edit-session-button").click();
    cy.get(".invalid-feedback").should(
      "contain.text",
      "Please input valid JSON"
    );
    cy.getDataCy("edit-env-variables-input")
      .clear()
      .type("{'TEST': '1'}", { parseSpecialCharSequences: false });
    cy.getDataCy("edit-session-button").click();
    cy.getDataCy("close-cancel-button").click();
    cy.getDataCy("get-back-session-view").click();
  });
